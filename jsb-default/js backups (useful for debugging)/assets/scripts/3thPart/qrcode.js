...
r += u;
var f = QRUtil.getErrorCorrectPolynomial(h), g = new QRPolynomial(i[s], f.getLength() - 1).mod(f);
a[s] = new Array(f.getLength() - 1);
for (l = 0; l < a[s].length; l++) {
var R = l + g.getLength() - a[s].length;
a[s][l] = R >= 0 ? g.get(R) : 0;
}
}
var d = 0;
...
for (l = 0; l < o; l++) for (s = 0; s < e.length; s++) l < i[s].length && (c[Q++] = i[s][l]);
for (l = 0; l < n; l++) for (s = 0; s < e.length; s++) l < a[s].length && (c[Q++] = a[s][l]);
return c;
};
...
for (var o = r.length / 3, n = new Array(), i = 0; i < o; i++) for (var a = r[3 * i + 0], s = r[3 * i + 1], u = r[3 * i + 2], h = 0; h < a; h++) n.push(new QRRSBlock(s, u));
return n;
};
